...
def find_duplicate(filePath):
    ...
    with open(base_file, mode='r', encoding='utf-8') as csv_file:
        ...
        for row in reader:
            content.setdefault(
                row['Virtual Terminal ID'],
                []).append(row['Device SN'])
    ...
    with open(f"{filePath}/multiple_sn_per_tid.csv", mode='w',
        ...
        fieldnames = ['Virtual Terminal ID', 'count', 'Device SN']
        writer = csv.DictWriter(csv_file, fieldnames=fieldnames)

        writer.writeheader()
        for key, value in filtered.items():
            writer.writerow({
                'Virtual Terminal ID': key,
                'count': len(value),
                'Device SN': value
            })

    return 1 if os.stat(f"{filePath}/multiple_sn_per_tid.csv") else 0
